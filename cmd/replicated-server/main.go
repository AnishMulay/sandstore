...
func createReplicatedServer(port string, nodeID string, otherNodes []cluster_service.Node) *server.ReplicatedServer {
	...
	fs := file_service.NewReplicatedFileService(ms, cs, cr, mr, ls, 8*1024*1024)
...
func main() {
	servers := []*server.ReplicatedServer{
		createReplicatedServer(":8080", "8080", []cluster_service.Node{
			{ID: "8081", Address: "localhost:8081", Healthy: true},
			{ID: "8082", Address: "localhost:8082", Healthy: true},
		}),
		createReplicatedServer(":8081", "8081", []cluster_service.Node{
			{ID: "8080", Address: "localhost:8080", Healthy: true},
			{ID: "8082", Address: "localhost:8082", Healthy: true},
		}),
		createReplicatedServer(":8082", "8082", []cluster_service.Node{
			{ID: "8080", Address: "localhost:8080", Healthy: true},
			{ID: "8081", Address: "localhost:8081", Healthy: true},
		}),
	}
	...
	for i, srv := range servers {
		...
		go func(s *server.ReplicatedServer, port int) {
			defer wg.Done()
			log.Printf("Starting replicated server on :808%d", port)
	...
	log.Println("All replicated servers started")
