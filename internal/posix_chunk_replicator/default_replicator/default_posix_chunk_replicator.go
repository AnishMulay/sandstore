...
type DefaultPosixChunkReplicator struct {
	clusterService cluster_service.ClusterService
	comm           communication.Communicator
	ls             log_service.LogService
}

func NewDefaultPosixChunkReplicator(
	clusterService cluster_service.ClusterService,
	comm communication.Communicator,
	ls log_service.LogService,
) *DefaultPosixChunkReplicator {
	return &DefaultPosixChunkReplicator{
...
func (r *DefaultPosixChunkReplicator) ReplicateChunk(chunkID string, data []byte, factor int) error {
	...
	if err != nil {
		return printernal.ErrInsufficientNodes
	}
	...
	for _, node := range targets {
		msg := communication.Message{
			From: r.comm.Address(),
			Type: communication.MessageTypePosixWriteChunk,
			Payload: communication.PosixWriteChunkRequest{
				ChunkID: chunkID,
				Data:    data,
			},
		}
	...
	if successCount == 0 {
		return printernal.ErrReplicationFailed
	}
...
func (r *DefaultPosixChunkReplicator) FetchReplicatedChunk(chunkID string) ([]byte, error) {
	...
	for _, node := range nodes {
		...
		msg := communication.Message{
			From: r.comm.Address(),
			Type: communication.MessageTypePosixReadChunk,
			Payload: communication.PosixReadChunkRequest{
				ChunkID: chunkID,
			},
		}
...
func (r *DefaultPosixChunkReplicator) DeleteReplicatedChunk(chunkID string) error {
	...
	for _, node := range nodes {
		...
		msg := communication.Message{
			From: r.comm.Address(),
			Type: communication.MessageTypePosixDeleteChunk,
			Payload: communication.PosixDeleteChunkRequest{
				ChunkID: chunkID,
			},
		}
