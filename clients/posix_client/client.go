...
func main() {
	...
	info := decode[pms.FileSystemInfo](r, ps.MsgPosixFsInfo, ps.FsInfoRequest{})
	log.Printf("FS Info: chunkSize=%d maxFile=%d maxName=%d\n", info.ChunkSize, info.MaxFileSize, info.MaxFilenameSize)

	stats := decode[pms.FileSystemStats](r, ps.MsgPosixFsStat, ps.FsStatRequest{})
	...
	r.expectFailure(ps.MsgPosixAccess, ps.AccessRequest{
	...
	r.expectFailure(ps.MsgPosixLookupPath, ps.LookupPathRequest{Path: "/home/alice/readme.txt"}, "old path should be gone after rename")
	log.Println("[PASS] Rename succeeded and old path invalidated")

	// Rmdir should fail while /tmp contains the file.
	r.expectFailure(ps.MsgPosixRmdir, ps.RmdirRequest{ParentID: rootID, Name: "tmp"}, "rmdir on non-empty dir should fail")
	...
	finalStats := decode[pms.FileSystemStats](r, ps.MsgPosixFsStat, ps.FsStatRequest{})
...
func (r *storyRunner) lookup(parentID, name string) string {
	return decode[string](r, ps.MsgPosixLookup, ps.LookupRequest{ParentID: parentID, Name: name})
}

func (r *storyRunner) lookupPath(path string) string {
	return decode[string](r, ps.MsgPosixLookupPath, ps.LookupPathRequest{Path: path})
}

func (r *storyRunner) mkdir(parentID, name string, mode, uid, gid uint32) *pms.Inode {
	return decode[*pms.Inode](r, ps.MsgPosixMkdir, ps.MkdirRequest{
		ParentID: parentID,
		Name:     name,
		Mode:     mode,
		UID:      uid,
...
func (r *storyRunner) create(parentID, name string, mode, uid, gid uint32) *pms.Inode {
	return decode[*pms.Inode](r, ps.MsgPosixCreate, ps.CreateRequest{
		ParentID: parentID,
		Name:     name,
		Mode:     mode,
		UID:      uid,
...
func (r *storyRunner) getAttr(inodeID string) *pms.Attributes {
	return decode[*pms.Attributes](r, ps.MsgPosixGetAttr, ps.GetAttrRequest{InodeID: inodeID})
}

func (r *storyRunner) setAttr(inodeID string, mode, uid, gid *uint32, atime, mtime *int64) *pms.Attributes {
	return decode[*pms.Attributes](r, ps.MsgPosixSetAttr, ps.SetAttrRequest{
		InodeID: inodeID,
		Mode:    mode,
		UID:     uid,
		GID:     gid,
...
func (r *storyRunner) access(inodeID string, uid, gid, mask uint32) {
	r.send(ps.MsgPosixAccess, ps.AccessRequest{
		InodeID:    inodeID,
		UID:        uid,
		GID:        gid,
		AccessMask: mask,
...
func (r *storyRunner) readBytes(inodeID string, offset, length int64) []byte {
	data := decode[[]byte](r, ps.MsgPosixRead, ps.ReadRequest{
		InodeID: inodeID,
		Offset:  offset,
		Length:  length,
	})
...
func (r *storyRunner) write(inodeID string, offset int64, data []byte) int64 {
	return decode[int64](r, ps.MsgPosixWrite, ps.WriteRequest{
		InodeID: inodeID,
		Offset:  offset,
		Data:    data,
	})
...
func (r *storyRunner) rename(srcParentID, srcName, dstParentID, dstName string) {
	r.send(ps.MsgPosixRename, ps.RenameRequest{
...
func (r *storyRunner) remove(parentID, name string) {
	r.send(ps.MsgPosixRemove, ps.RemoveRequest{ParentID: parentID, Name: name}, false)
}

func (r *storyRunner) rmdir(parentID, name string) {
	r.send(ps.MsgPosixRmdir, ps.RmdirRequest{ParentID: parentID, Name: name}, false)
}
...
func (r *storyRunner) collectDir(inodeID string, maxEntries int) []pms.DirEntry {
	...
	for {
		page := decode[readDirResponse](r, ps.MsgPosixReadDir, ps.ReadDirRequest{
			InodeID:    inodeID,
			Cookie:     cookie,
...
func (r *storyRunner) collectDirPlus(inodeID string, maxEntries int) []pms.DirEntryPlus {
	...
	for {
		page := decode[readDirPlusResponse](r, ps.MsgPosixReadDirPlus, ps.ReadDirPlusRequest{
			InodeID:    inodeID,
			Cookie:     cookie,
			MaxEntries: maxEntries,
		})
...
func registerPayloads(comm *grpccomm.GRPCCommunicator) {
	comm.RegisterPayloadType(ps.MsgPosixGetAttr, reflect.TypeOf(ps.GetAttrRequest{}))
	comm.RegisterPayloadType(ps.MsgPosixSetAttr, reflect.TypeOf(ps.SetAttrRequest{}))
	...
	comm.RegisterPayloadType(ps.MsgPosixAccess, reflect.TypeOf(ps.AccessRequest{}))
	...
	comm.RegisterPayloadType(ps.MsgPosixRemove, reflect.TypeOf(ps.RemoveRequest{}))
	comm.RegisterPayloadType(ps.MsgPosixRmdir, reflect.TypeOf(ps.RmdirRequest{}))
	comm.RegisterPayloadType(ps.MsgPosixRename, reflect.TypeOf(ps.RenameRequest{}))
	comm.RegisterPayloadType(ps.MsgPosixReadDir, reflect.TypeOf(ps.ReadDirRequest{}))
	comm.RegisterPayloadType(ps.MsgPosixReadDirPlus, reflect.TypeOf(ps.ReadDirPlusRequest{}))
	comm.RegisterPayloadType(ps.MsgPosixFsStat, reflect.TypeOf(ps.FsStatRequest{}))
